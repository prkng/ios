...
class MGLMapViewController: MapViewController, MGLMapViewDelegate, UIGestureRecognizerDelegate {
    ...
    let ZOOM_DEFAULT_APP_LAUNCH: Double = 17.0
    let ZOOM_DEFAULT: Double = 16.0
    let ZOOM_GENERAL_THRESHOLD: Double = 15.0
    let ZOOM_BUTTON_THRESHOLD: Double = 17.0
    let ZOOM_BIG_BUTTON_THRESHOLD: Double = 18.0
    let ZOOM_GARAGE_THRESHOLD: Double = 14.0
    let ZOOM_FIND_CAR_THRESHOLD: Double = 13.0
...
//            return
    ...
    func singleTapOnMap(tapRec: UITapGestureRecognizer) {
        ...
        let loopThroughLines = mapView.zoomLevel < ZOOM_BUTTON_THRESHOLD
...
//                && !annotation.isClusterAnnotation
    ...
    override func updateAnnotations(completion: ((operationCompleted: Bool) -> Void)) {
        ...
        } else if self.mapView.zoomLevel >= ZOOM_GENERAL_THRESHOLD
            || (self.mapMode == .Garage && self.mapView.zoomLevel >= ZOOM_GARAGE_THRESHOLD)
            || (self.mapView.zoomLevel >= ZOOM_FIND_CAR_THRESHOLD && self.mapMode == .CarSharing && self.delegate?.carSharingMode() == .FindCar) {
...
//            }
    ...
    func annotationForSpot(spot: ParkingSpot, selected: Bool, addToMapView: Bool) -> (MGLLineParkingSpot, [GenericMGLAnnotation]) {
        ...
        if (mapView.zoomLevel >= ZOOM_BUTTON_THRESHOLD) {
...
//            self.mapView.removeAnnotation(annotation as! MGLAnnotation)
//        }
        self.mapView.removeAnnotations(self.annotations as! [MGLAnnotation])
        self.annotations = []
    }
    ...
    override func setDefaultMapZoom() {
        switch self.mapMode {
        case .Garage:
            self.mapView.setZoomLevel(self.ZOOM_GARAGE_THRESHOLD, animated: true)
        case .StreetParking:
            self.mapView.setZoomLevel(self.ZOOM_DEFAULT, animated: true)
        case .CarSharing:
