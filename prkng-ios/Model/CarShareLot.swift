...
class CarShareLot: NSObject {
    ...
    func mapPinImageAndReuseIdentifier(_ selected: Bool) -> (UIImage, String) {
        ...
        //set color
        UIGraphicsGetCurrentContext()?.setFillColor(pinColor.cgColor)
        UIGraphicsGetCurrentContext()?.fill(bubbleRect)
        
        //drawthe round P
        //P is 14pt x 14pt with 6.5pt margin around
        roundPImage.draw(in: CGRect(x: 6.5, y: 6.5, width: 14, height: 14))
        
        //draw the label text
        label.drawText(in: CGRect(x: 6.5 + 14 + 6.5, y: (bubbleHeight-labelHeight)/2, width: labelWidth, height: labelHeight))
        ...
        anchorImage.draw(in: CGRect(x: bubbleWidth/2 - 3, y: bubbleHeight, width: anchorImage.size.width, height: anchorImage.size.height))
    ...
    func calloutView() -> (UIView, UIView) {
        ...
        let maximumTotalWidth = Int(UIScreen.main.bounds.width) - 40
        ...
        rightView.setImage(UIImage(named:"btn_directions_red"), for: UIControlState())
